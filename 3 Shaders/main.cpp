...
void main()
{
    float b = atan(aPos.y, aPos.x);
    float dist = distance(aPos, vec3(0, 0, 0));
    gl_Position = vec4(cos(b+alpha)*dist, -sin(b+alpha)*dist, 0, 1.0);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform1f(alphaLocation, timeValue);
